...
pub fn Home() -> impl IntoView {
    ...
    let scrolling:Closure<dyn FnMut()> = Closure::new(move|| {
        let scroll_position = window_ref.scroll_y().unwrap_or(0.0);
        let threshold:f64 = 50.00;
        if scroll_position > threshold {
            let indicator:web_sys::HtmlElement=document.get_element_by_id("root_path_indicator_nav").unwrap().dyn_into().unwrap();
             indicator.style().set_property("display","none").unwrap();
        }else{
            let indicator:web_sys::HtmlElement=document.get_element_by_id("root_path_indicator_nav").unwrap().dyn_into().unwrap();
            indicator.style().remove_property("display").unwrap();
        }
    });
    window.set_onscroll(Some(scrolling.as_ref().unchecked_ref()));
    scrolling.forget();
        ...
        <div class="min-h-[100vh] bg-[#1d1d1d]">
            ...
            <div class="flex">
                ...
                <div class="mt-[48px] flex-grow mb-[22px] text-[#999999]">
                    ...
                    <div id="folder_view">
                        // folder view nav
                        <div id="folder_view_nav" class="fixed overflow-hidden left-[200px] right-0 top-[46px] h-[57px]">
                            <div class="flex w-full bg-[#151515]">
                            ...
                            <div id="root_path_indicator_nav" class="w-full flex items-center h-[22px] text-[13px] bg-[#1d1d1d] text-[#999999] px-[16px] root_path_indicator">
                            </div>
                        </div>
